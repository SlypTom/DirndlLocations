...
export interface RentalFormPayload {
  customerId: string;
  dateDebut: string;
  dateFin: string;
  prixTotal: number;
...
export default function RentalForm({
  ...
  submitLabel: string;
  savingLabel: string;
  onSubmit: (payload: RentalFormPayload) => Promise<string | null | void>;
  ...
  const [dateDebut, setDateDebut] = useState(initialValues.dateDebut);
  const [dateFin, setDateFin] = useState(initialValues.dateFin);
  const [prixOverride, setPrixOverride] = useState<string | null>(
    initialValues.prixTotal || null
  );
  ...
  useEffect(() => {
    async function load() {
      const itemsQuery =
        initialValues.itemIds.length > 0
          ? supabase
              .from("items")
              .select("*")
              .or(
                `statut.in.(disponible,loue),id.in.(${initialValues.itemIds.join(",")})`
              )
              .order("reference")
          : supabase
              .from("items")
      ...
      const [customersRes, itemsRes, windowsRes] = await Promise.all([
        ...
        supabase
          .from("rental_items")
          .select(
            "item_id, rental_id, rentals!inner(date_debut, date_fin_prevue, statut)"
          )
      ...
      if (windowsRes.data) {
        setOccupiedWindows(
          (
            ...
            .filter((w) => w.rental_id !== excludeRentalId)
            .map((w) => ({
              item_id: w.item_id,
              rental_id: w.rental_id,
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const submitError = await onSubmit({
      ...
      dateDebut,
      dateFin,
      prixTotal: Number(displayedPrix) || suggestedTotal,
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      <label className="block text-sm font-medium">
        ...
        <div className="mt-1 flex gap-2">
          ...
          <Link
            href={`/customers/new?returnTo=${encodeURIComponent(newCustomerReturnTo)}`}
